...
- [ ] Capture the complete submitted line, current provider path, and PowerShell HistoryId when available; emit a versioned nonce-tagged private OSC frame with bounded UTF-8/base64, window/session identity, reason, and callback ID.
...
- [ ] Unit fixtures cover shell detection, safe/conflicting args, Unicode/base64 line/location, cwd fingerprinting/raw-path discard, wrong nonce, malformed/oversized/truncated frames, every chunk boundary, duplicate event IDs, byte ordering, and byte preservation.
