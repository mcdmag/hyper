...
5. First use discloses the exact required context (failed line, PowerShell family/version, operating system, opaque attempt ID, and cwd fingerprint), optional raw cwd/Git metadata, and the separately opt-in handling of secret-looking input before offering ChatGPT/Codex browser sign-in.
...
Privacy consent is versioned non-secret per-install state at app.getPath('userData')/nli/preferences.json: privacyNoticeVersion=1 plus cwd/Git disclosure choices and the separately default-off secret-looking-input sharing choice. It is shared across profiles/windows, never stores input or command text, and can be revoked from Natural Language Setup. Revocation cancels active turns and returns to consent-required. Keyring unavailable, userData/CODEX_HOME write failure, and keyring-only policy rejection produce distinct accessible errors with no plaintext fallback.
