...
- [ ] Define a strict versioned response union: kind=plan with bounded summary and one-to-three command options, or kind=clarification with one bounded question and two-to-three answer choices. A plan option has stable ID, label, rationale, assumptions, one exact shell-native payload, and purpose.
...
- [ ] Add local secret-pattern screening before transmission; versioned per-install first use discloses exact fields and optional cwd/Git metadata, secret-looking input requires explicit share consent or is refused, and reset revokes consent/cancels active turns.
